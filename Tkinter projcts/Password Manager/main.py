...
def generate_password():
    ...
    password_entry.insert(index=0, string=password_string)
    copy(password_string) # Copy password on the clipboard
    hide_button.configure(command=hide_password, text="Hide")

# ------------------ Save Password ------------------ #
def write_to_file(website:str, name_email:str, password:str):
    with open("Passwords.txt", mode="a") as file:
        text_to_write = website + " | " + name_email + " | " + password + "\n"
...
def save_password():
    ...
    if len(website) < 2 or len(email_username) < 5 or len(password) < 5: # don't let user save if nothing is typed
        filed_to_fill = ""
        if len(website) < 2:
            filed_to_fill = "Website"
        elif len(email_username) < 5:
        ...
        add_data = messagebox.askokcancel(title=website, message=f"Email: {email_username}\nPassword: {password}\nPress OK to add.")
...
website_entry = CTkEntry(master=app, width=450, text_color=YELLOW, font=(FONT, 15, "bold"), bg_color=RICH_BLACK, fg_color=TRANSPARENT, border_color=VIOLET, placeholder_text= "Enter Any Website", placeholder_text_color=VIOLET)
...
hide_button = CTkButton(master=app, width=155, bg_color=RICH_BLACK, fg_color=TRANSPARENT, border_color=LIGHT_CORAL, hover_color=LIGHT_CORAL, text_color=YELLOW, text="Hide", border_width=1, font=(FONT, 15, "bold"), command=hide_password)
# Placing the buttons
